...
HEALTH_KNOWLEDGE_BASE = {
    'питание': {
        'вода':
        'Пейте 8 стаканов воды в день (около 2 литров). Вода помогает пищеварению и выводит токсины.',
        'белок':
        'Взрослому человеку нужно 0.8-1г белка на кг веса. Источники: мясо, рыба, яйца, бобовые.',
        'витамины':
        'Получайте витамины из разнообразной пищи: фрукты, овощи, орехи, зелень.',
        'завтрак':
        'Завтракайте в течение часа после пробуждения. Это запускает метаболизм.',
        'сахар':
        'Ограничьте сахар до 25г в день. Избыток сахара ведет к диабету и ожирению.',
    },
    'фитнес': {
        'кардио':
        '150 минут умеренной активности в неделю. Бег, плавание, велосипед.',
        'силовые':
        'Тренируйте мышцы 2-3 раза в неделю. Используйте вес тела или гантели.',
        'растяжка':
        'Делайте растяжку после тренировок. Это предотвращает травмы.',
        'разминка':
        'Всегда начинайте с 5-10 минут разминки перед тренировкой.',
        'отдых':
        'Давайте мышцам отдыхать 48 часов между силовыми тренировками.',
    },
    'сон': {
        'продолжительность':
        'Взрослым нужно 7-9 часов сна. Подросткам - 8-10 часов.',
        'режим':
        'Ложитесь и вставайте в одно время. Это улучшает качество сна.',
        'экраны':
        'Не смотрите в экраны за час до сна. Синий свет мешает засыпанию.',
        'температура': 'Оптимальная температура для сна 18-20°C.',
    },
    'психология': {
        'стресс':
        'Практикуйте дыхательные упражнения, медитацию, йогу для снижения стресса.',
    ...
    'гигиена': {
        ...
        'зубы':
        'Чистите зубы 2 раза в день по 2 минуты. Используйте зубную нить.',
    }
}
...
def handle_query():
    ...
    if is_command(user_input):
        ...
        # Обработаем вход как запрос к LLM
...
def chat():
    ...
    is_command = user_message.startswith(
        '/profile') or user_message.startswith(
            '/medication') or user_message.startswith(
                '/reminder') or user_message.startswith('/knowledge')
    ...
    try:
        ...
        if knowledge_context:
            messages.append({
                ...
                'system',
                'content':
                f"Релевантная информация из базы знаний: {knowledge_context}"
            })
        ...
        prompt_parts = []
        for msg in messages:
            role_prefix = "Система" if msg['role'] == 'system' else (
                "Пользователь" if msg['role'] == 'user' else "Ассистент")
        ...
        assistant_message = markdown.markdown(
            raw_message, extensions=['nl2br', 'fenced_code'])

        medical_keywords = [
            'симптом', 'болезн', 'боль', 'боли', 'болит', 'болят', 'болел',
            'болела', 'болело', 'болели', 'болею', 'болеешь', 'болеет',
            'болеем', 'болеете', 'болеют', 'болеть', 'переболел', 'болен',
            'больна', 'больны', 'больной', 'больным', 'больного', 'больных',
            'заболевание', 'заболел', 'заболела', 'заболели', 'заболеть',
            'заболеваю', 'заболевают', 'температур', 'жар', 'лихорадка',
            'давлени', 'диагноз', 'лечен', 'лечить', 'лечу', 'лечусь',
            'вылечить', 'излечить', 'препарат', 'таблетк', 'лекарств',
            'медикамент', 'врач', 'доктор', 'специалист', 'госпитал', 'клиник',
            'больниц', 'поликлиник', 'анализ', 'обследован', 'терапи',
            'инфекц', 'вирус', 'бактери', 'ковид', 'covid', 'коронавирус',
            'коронавируса', 'коронавирусом', 'простуд', 'простыл', 'простыла',
            'простыли', 'простужен', 'простужена', 'простужены', 'орви', 'орз',
            'грипп', 'гриппом', 'гриппа', 'ангина', 'ангиной', 'ангину',
            'ангины', 'бронхит', 'бронхита', 'бронхитом', 'пневмония',
            'пневмонии', 'пневмонией', 'отит', 'отита', 'отитом', 'гастрит',
            'гастрита', 'гастритом', 'язва', 'язвы', 'язвой', 'язву', 'кашел',
            'кашля', 'кашляю', 'кашляет', 'насморк', 'чихан', 'чихаю',
            'недомогаю', 'недомогание', 'чувствую себя плохо',
            'плохо себя чувствую', 'головн', 'голова', 'головой', 'голову',
            'горло', 'горла', 'горлом', 'горле', 'ухо', 'уха', 'ухом', 'уши',
            'ушей', 'ушах', 'кожа', 'кожи', 'кожей', 'коже', 'нога', 'ноги',
            'ногой', 'ногу', 'ног', 'ногами', 'колено', 'колена', 'коленом',
            'колени', 'коленей', 'рука', 'руки', 'рукой', 'руку', 'рук',
            'руками', 'палец', 'пальца', 'пальцем', 'пальцы', 'пальцев',
            'плечо', 'плеча', 'плечом', 'плечи', 'плеч', 'спина', 'спины',
            'спиной', 'спине', 'спину', 'зуб', 'зуба', 'зубом', 'зубы',
            'зубов', 'зубами', 'глаз', 'глаза', 'глазом', 'глазу', 'глаза',
            'глаз', 'сыпь', 'сыпи', 'сыпью', 'желудок', 'желудка', 'живот',
            'живота', 'сердц', 'сердечн', 'инфаркт', 'инфаркта', 'инфарктом',
            'инсульт', 'инсульта', 'инсультом', 'рак', 'рака', 'раком',
            'опухоль', 'опухоли', 'опухолью', 'онкол', 'химиотерапи',
            'гепатит', 'гепатита', 'гепатитом', 'цирроз', 'почечн', 'почки',
            'печен', 'печени', 'операци', 'операция', 'операции', 'операцию',
            'хирург', 'анестези', 'переливание', 'переливани', 'сахар',
            'диабет', 'астма', 'аллерги', 'артрит', 'артрита', 'артритом',
            'остеохондроз', 'остеохондроза', 'остеохондрозом', 'ломит',
            'поясниц', 'воспален', 'воспалена', 'воспалены', 'воспаление',
            'воспалился', 'воспалилась', 'воспалилось', 'воспалились', 'отек',
            'опухоль', 'рана', 'раны', 'травма', 'травмы', 'перелом',
            'переломил', 'переломила', 'сломал', 'сломала', 'сломали', 'слом',
            'ушиб', 'ушибла', 'ушибли', 'порез', 'порезал', 'порезала',
            'порезали', 'подвернул', 'подвернула', 'растяжение', 'растяжени',
            'беспокоит', 'беспокоят', 'боль в', 'болит в', 'боли в', 'тошнит',
            'тошнота', 'рвота', 'понос', 'диарея', 'запор', 'слабость',
            'слабый', 'усталость', 'устал', 'устала', 'бессонниц', 'депресси',
            'стресс', 'панич', 'тревож', 'мигрен', 'судорог', 'онемени',
            'немеет', 'головокружен', 'кружится', 'обморок', 'кровотечени',
            'кровь', 'кровит', 'гной', 'выделени', 'отравлени', 'отравился'
        ]

        disclaimer = "\n\n⚠️ Я ИИ, а не врач. Пожалуйста, проконсультируйтесь со специалистом."
        needs_disclaimer = any(keyword in user_message.lower()
                               or keyword in assistant_message.lower()
...
def calculate_next_medication_time(med_time):
    ...
    next_time = now.replace(hour=med_hour,
                            minute=med_minute,
                            second=0,
                            microsecond=0)
...
def handle_medication_command(message):
    ...
    if len(parts) == 1:
        ...
        try:
            index = int(parts[2]) - 1
            if 'medications' in session and 0 <= index < len(
                    session['medications']):
...
def handle_reminder_command(message):
    ...
    if len(parts) == 1:
        ...
        try:
            index = int(parts[2]) - 1
            if 'reminders' in session and 0 <= index < len(
                    session['reminders']):
...
def suggest_command(message):
    ...
    if any(pattern in message_lower for pattern in medication_patterns):
        ...
        if time_match:
            ...
            through_match = re.search(r'через (\d+) (минут|час)',
                                      message_lower)
            if through_match:
        ...
        for i, word in enumerate(med_words):
            if any(p in word.lower()
                   for p in ['лекарств', 'таблетк', 'препарат']):
...
📝 Формат: /medication [название] [время_HH:MM]"""
    ...
    # Проверяем на обычные напоминания
        ...
        reminder_text = message.replace('напомни мне',
                                        '').replace('напомни', '').strip()
    ...
    if 'reminders' in session:
        ...
        for idx, reminder in enumerate(session.get('reminders', [])):
            if reminder['time'] == current_time:
                ...
                if reminder.get('repeat', 'один раз').lower() in [
                        'один раз', 'однократно', 'разовое'
                ]:
    ...
    try:
        ...
        if reminder_type == 'medication':
            if 'medications' in session and 0 <= index < len(
                ...
                return jsonify({
                    'status': 'success',
                    'message': f"Удалено: {removed['name']}"
                })
        else:
            if 'reminders' in session and 0 <= index < len(
                    session['reminders']):
                ...
                return jsonify({
                    'status': 'success',
                    'message': f"Удалено: {removed['text']}"
                })
